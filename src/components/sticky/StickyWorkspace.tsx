...
export function StickyWorkspace({ initialData, mode, systemMessage }: StickyWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        setWorkspace(JSON.parse(stored) as StickyWorkspaceData);
      } catch {
  ...
  const commandItems: CommandItem[] = [
    ...
    ...(activeList
      ? [
          ...
          },
          {
            ...
            detail: `${completedTasks.length} completed ${plural(completedTasks.length, "sticky", "stickies")}`,
      ...
      ? [
          selectedTask.isCompleted
            ...
            : {
                ...
                kind: "action" as const,
                title: "Complete selected sticky",
                detail: selectedTask.title,
              ...
              },
          {
            ...
            kind: "action" as const,
            title: "Duplicate selected sticky",
            detail: selectedTask.title,
    ...
    ...TASK_VIEW_ORDER.map((filter) => ({
      id: `view-${filter}`,
      kind: "action" as const,
      title: `Show ${TASK_VIEW_LABELS[filter].toLowerCase()} stickies`,
    ...
    },
    {
      ...
      detail: taskSortMode === "due" ? "Already active" : "Earliest scheduled stickies first",
    ...
    },
    {
      ...
      detail: colorMode === "system" ? "Already active" : "Follow this device",
      keywords: "theme color mode system",
      run: () => setColorMode("system"),
    },
    {
      id: "preference-light",
      kind: "preference",
  ...
  function deleteList(list: StickyList) {
    ...
    pushToast({
      title: "List deleted",
      body: `${list.name} and ${deletedTasks.length} ${plural(deletedTasks.length, "sticky", "stickies")} can be restored.`,
      actionLabel: "Undo",
      onAction: () => {
        if (undoUsed) {
          return;
  ...
  function addSubtask(task: StickyTask, title: string) {
    ...
    if (recurrenceByTask.has(task.id)) {
      pushToast({
        title: "Repeating stickies stay simple",
        ...
        void deleteRequest.then((deleteSaved) => {
          ...
          if (!restoreBefore.tasks.some((task) => task.id === subtask.taskId)) {
            pushToast({
              ...
              body: "The parent sticky is no longer available.",
            });
            return;
          }
  ...
  function toggleRecurrence(task: StickyTask, enabled: boolean) {
    ...
    if (enabled) {
      if ((subtasksByTask.get(task.id) ?? []).length > 0) {
        pushToast({
          title: "Repeating stickies cannot have subtasks",
  ...
  function advanceRecurringCatchUps(targets = recurringCatchUps) {
    ...
    pushToast({
      ...
      body: `${targets.length} ${plural(targets.length, "sticky", "stickies")} advanced across ${totalSkipped} ${plural(totalSkipped, "missed repeat")}.`,
    });
  }
  ...
  return (
    <main className={`sticky-app density-${density} tone-${colorMode}`}>
      <DndContext
      ...
      >
        ...
        <section className="task-stage" aria-label="Sticky workspace">
          ...
          <header className="workspace-topbar">
            ...
            <div className="workspace-tools">
              ...
              <div className="preference-controls" aria-label="Workspace preferences">
                ...
                <div className="segmented-control icon-only" aria-label="Color mode">
                  <button
                    type="button"
                    className={colorMode === "system" ? "active" : ""}
            ...
            <div className="recurrence-catchup-banner">
              <div>
                ...
                <span>
                  ...
                  {plural(recurringCatchUps.length, "sticky", "stickies")} behind schedule
                </span>
              </div>
          ...
          <section className="task-lane" aria-label="Active stickies">
            {activeTasks.length ? (
              <SortableContext
...
function TaskDetailsPanel({
  ...
  if (!task) {
    return (
      <aside className="details-panel empty-details" aria-label="Sticky details">
        <div className="pulse-panel">
          ...
          <div className="pulse-balance">
            ...
            <div>
              <Repeat2 size={16} />
              <span>{pulse.recurringCount} repeating {plural(pulse.recurringCount, "sticky", "stickies")}</span>
            </div>
          </div>
  ...
  return (
    <aside className="details-panel" aria-label="Sticky details">
