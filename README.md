...
the local and deployed Vercel Cron schedule, the intended custom-domain DNS
record, the local verification workflow and release branch, the local Git remote
and Vercel Git integration state, the required Vercel production env names, and,
when `SUPABASE_ACCESS_TOKEN` is present locally, Supabase Auth site URL and
