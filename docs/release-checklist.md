...
- `npm run launch:check` succeeds against the local Vercel link, production
  ...
  headers, install manifest, cron guard, Vercel Cron schedule, CI workflow,
  release branch, Git remote handoff, Vercel Git integration, Vercel production
  env names, and Supabase Auth URL/callback settings when a local Management API
  token is supplied.
