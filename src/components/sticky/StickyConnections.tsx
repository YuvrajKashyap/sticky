...
import { BellRing, Check, Copy, ExternalLink, KeyRound, Send, Smartphone, Unplug, X } from "lucide-react";
...
export function StickyConnections({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const createPokeCredential = useMutation({
    mutationFn: () => client!.request<{ token: string; mcpUrl: string }>("/api/v1/credentials", {
      method: "POST",
      body: JSON.stringify({ name: "Poke", provider: "poke", providerUserId: null, scopes: ["tasks:read", "tasks:write", "tasks:destructive"] }),
    }),
